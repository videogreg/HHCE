...
export const checkConstraints = (
  ...
  // Helper to resolve active cleaners for a visit
  const getVisitCleaners = (visit: Visit): Cleaner[] => {
    let ids = visit.assignedCleanerIds || [];
    if (ids.length === 0) {
      const team = teams.find(t => t.id === visit.assignedTeamId);
      if (team) ids = team.cleanerIds;
    }
    return cleaners.filter(c => ids.includes(c.id) && c.active);
  };
  ...
  // NEW: Cross-visit day-level scheduling constraints
  const dayVisitsMap = new Map<string, Visit[]>();
  visits.filter(v => !v.cancelled).forEach(v => {
    const list = dayVisitsMap.get(v.date) || [];
    list.push(v);
    dayVisitsMap.set(v.date, list);
  });

  dayVisitsMap.forEach((dayVisits) => {
    const clientToDrivers = new Map<string, Set<string>>();
    const nonDriverToDrivers = new Map<string, Set<string>>();

    dayVisits.forEach(visit => {
      const visitCleaners = getVisitCleaners(visit);
      const drivers = visitCleaners.filter(c => c.isDriver);

      // Rule 1: Multiple drivers on same visit
      if (drivers.length >= 2) {
        const driverNames = drivers.map(d => d.name).join(', ');
        violations.push({
          id: hashString(`${visit.id}-multiDriver-${drivers.map(d => d.id).sort().join('-')}`),
          visitId: visit.id,
          message: `Multiple drivers assigned (${driverNames}). A driver may only work with non-driver cleaners.`,
          severity: 'error'
        });
      }

      // Track route ownership for single-driver visits only
      if (drivers.length === 1) {
        const driver = drivers[0];
        if (!clientToDrivers.has(visit.clientId)) {
          clientToDrivers.set(visit.clientId, new Set());
        }
        clientToDrivers.get(visit.clientId)!.add(driver.id);

        visitCleaners.filter(c => !c.isDriver).forEach(nd => {
          if (!nonDriverToDrivers.has(nd.id)) {
            nonDriverToDrivers.set(nd.id, new Set());
          }
          nonDriverToDrivers.get(nd.id)!.add(driver.id);
        });
      }
    });

    // Rule 2: Client duplication across driver routes
    clientToDrivers.forEach((driverIds, clientId) => {
      if (driverIds.size > 1) {
        const client = clients.find(c => c.id === clientId);
        const driverNames = Array.from(driverIds)
          .map(did => cleaners.find(c => c.id === did)?.name || 'Unknown')
          .join(', ');
        dayVisits
          .filter(v => v.clientId === clientId)
          .forEach(visit => {
            const vCleaners = getVisitCleaners(visit);
            const vDrivers = vCleaners.filter(c => c.isDriver);
            if (vDrivers.length === 1 && driverIds.has(vDrivers[0].id)) {
              violations.push({
                id: hashString(`${visit.id}-clientDup-${clientId}-${Array.from(driverIds).sort().join('-')}`),
                visitId: visit.id,
                message: `${client?.name || 'Client'} appears in multiple driver routes (${driverNames}). Each client should be assigned to only one driver per day.`,
                severity: 'error'
              });
            }
          });
    ...
    // Rule 3: Non-driver cleaner shared across drivers
    nonDriverToDrivers.forEach((driverIds, nonDriverId) => {
      if (driverIds.size > 1) {
        const nonDriver = cleaners.find(c => c.id === nonDriverId);
        const driverNames = Array.from(driverIds)
          .map(did => cleaners.find(c => c.id === did)?.name || 'Unknown')
          .join(', ');
        dayVisits.forEach(visit => {
          const vCleaners = getVisitCleaners(visit);
          const vDrivers = vCleaners.filter(c => c.isDriver);
          const hasNonDriver = vCleaners.some(c => c.id === nonDriverId);
          if (hasNonDriver && vDrivers.length === 1 && driverIds.has(vDrivers[0].id)) {
            violations.push({
              id: hashString(`${visit.id}-cleanerShare-${nonDriverId}-${Array.from(driverIds).sort().join('-')}`),
              visitId: visit.id,
              message: `${nonDriver?.name || 'Cleaner'} is assigned to multiple drivers (${driverNames}). Additional cleaners must stay with one driver per day.`,
              severity: 'error'
            });
          }
        });
