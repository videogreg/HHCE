              ...
              {!visit.cancelled && (
                <div className="flex flex-wrap gap-1.5">
                  {assignedCleaners.slice(0, 5).map(c => (
                    <span key={c!.id} className="text-[10px] font-bold px-2 py-1 rounded-lg bg-slate-100 text-slate-600 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: c!.color || '#94a3b8' }} />
                      {c!.name}
                    </span>
                  ))}
                  {assignedCleaners.length > 5 && (
                    <span className="text-[10px] font-bold px-2 py-1 rounded-lg bg-slate-100 text-slate-500">
                      +{assignedCleaners.length - 5} more
                    </span>
                  )}
