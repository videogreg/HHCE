...
export interface CleanerRouteViewProps {
  cleaner: Cleaner;
  date: string;
  visits: Visit[];
  cleaners: Cleaner[];
...
export const CleanerRouteView: React.FC<CleanerRouteViewProps> = ({
  ...
  const dayVisits = useMemo(() =>
    visits.filter(v => v.date === date && !v.cancelled).sort((a, b) => a.startTime.localeCompare(b.startTime)),
  [visits, date]);
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm animate-slide-up max-w-3xl mx-auto">
      ...
      {myVisits.length === 0 && !loading && (
        <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm text-center">
          ...
          <p className="text-slate-400 text-xs mt-1">Enjoy the day off!</p>
        </div>
      )}
      ...
      {myVisits.length > 0 && !loading && (
        <>
          {/* Driver / Pickup info */}
          {cleaner.isDriver && (
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 space-y-1 mb-4">
              <p className="text-blue-800 text-xs font-bold flex items-center gap-2">
                <Car size={14} /> Driving today
              </p>
              {teamHours.length > 0 && (
          ...
          {!cleaner.isDriver && hasDriverPickup && myDriver && (
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 space-y-2 mb-4">
          ...
          <div className="space-y-3 mb-4">
            <h2 className="text-sm font-bold text-slate-800 flex items-center gap-2">
              ...
              {cleaner.isDriver ? 'Route' : myDriver ? `${myDriver.name}'s Route` : 'Route'}
                  ...
                  <div key={tm.name} className={`flex items-center justify-between p-3 rounded-xl border ${tm.name === cleaner.name ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-100'}`}>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {tm.name === cleaner.name && <span className="text-[10px] text-green-600 font-bold ml-1.5">(Viewed)</span>}
